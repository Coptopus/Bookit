                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: InkWell(
                          onTap: () async{
                            if (email.text == "") {
                              AwesomeDialog(context: context, dialogType: DialogType.error, animType: AnimType.bottomSlide, title: "To Reset Password", desc: "Please enter your email in the 'Email' and try again.",  btnOkOnPress: () {},).show();
                              return;
                            }
                            try {
                            await FirebaseAuth.instance.sendPasswordResetEmail(email: email.text);  
                            if (!context.mounted) {return;}
                            AwesomeDialog(context: context, dialogType: DialogType.info, animType: AnimType.bottomSlide, title: "Reset Password", desc: "Please check your email for the password reset link.",  btnOkOnPress: () {},).show();
                            } catch (e) {
                            AwesomeDialog(context: context, dialogType: DialogType.error, animType: AnimType.bottomSlide, title: "Reset Password", desc: "Please make sure you entered the correct email",  btnOkOnPress: () {},).show();
                              }
                            }, 
                ...
                ButtonAuth(label: "Login", onPressed: () async {
                  ...
                  try {
                    ...
                    if (credential.user!.emailVerified) {
                      ...
                      if (!context.mounted) {return;}
                      AwesomeDialog(context: context, dialogType: DialogType.info, animType: AnimType.bottomSlide, title: "Almost there!", desc: "Please check your email for the verification link.",  btnOkOnPress: () {}, btnCancelText: "Re-send verfication link", btnCancelOnPress: () {FirebaseAuth.instance.currentUser!.sendEmailVerification();},).show();
                    }
                  } on FirebaseAuthException catch (e) {
                    if (e.code == 'user-not-found') {
                      if (!context.mounted) {return;}
                      AwesomeDialog(context: context, dialogType: DialogType.error, animType: AnimType.bottomSlide, title: "Error", desc: "No user found for that email.").show();
